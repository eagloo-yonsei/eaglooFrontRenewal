...
export default function CustomRoomModalProvider({
  isLoggedIn,
  userInfo,
  ...
  const [roomDescriptionInput, setRoomDescriptionInput] = useState<string>('');
  ...
  function enterRoom(roomId: string) {
    ...
    if (isLoggedIn) {
      router.push(`/entry?roomType=${RoomType.CUSTOM}&roomId=${roomId}`);
    } else {
      toastRequestLoginMessage();
      router.push('/login');
    }
    ...
    await axios
      ...
      .then((response) => {
        if (response.data.success) {
          router.push(
            `/entry?roomType=${RoomType.CUSTOM}&roomId=${response.data.roomId}`
          );
        } else {
          setCreatingRoom(false);
          toastErrorMessage(
            response.data.message || '방을 만드는 도중 오류가 발생했어요.'
          );
        }
      })
      .catch((error) => {
